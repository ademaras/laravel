...
class AboutController extends Controller
{
    public function aboutPost(Request $request)
    {
        ...
        if (!$user->businessCard()->exists()) {
            $user->businessCard()->create([
                'corporate_id' => $user->id,
            ]);
        }

        if (!$user->corporateBasic()->exists()) {
            $user->corporateBasic()->create([
                'card_id' => $user->businessCard->id,
            ]);
        }
